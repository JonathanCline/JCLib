...
struct GuardedState
{
	...
	// Set once release is called
	bool release_called = false;

	// Set once good is called
...
struct GuardedObject
{
	...
	void release()
	{
		if (this->gstate)
		{
			this->gstate->release_called = true;
		};
		this->gstate = nullptr;
	};
...
static_assert(sizeof(guard_type) == 8, "statiful guarded object size mismatch");
...
int subtest_release()
{
...

	{
		...
		ASSERT(_state.release_called, "release did not set state to dead");
...
int subtest_construction()
{
	...
	// Test construction
	{
		...
		ASSERT(!_gstate.release_called, "invalid preconditions");

		guard_type _g{ _gstate };

		ASSERT(!_gstate.release_called, "set to dead at construction");
	};
...
int subtest_reset()
{
	...
	guard_type _g{ _gstate };
	ASSERT(!_gstate.release_called, "bad preconditions");
	ASSERT(!_gstate.reset_called, "bad preconditions");

	_g.reset();

	ASSERT(_gstate.release_called, "reset did not set state to dead");
